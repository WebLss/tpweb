...
function markdown_to_html($markdown)
{
    ...
    // markdown转html
    $parser = new Org\HyperDown\Parser();
...
function RU($url){
    ...
    //兼容 github/:aid\d=>Index/github
    if(preg_match('/\/Home\/Index\/github\/aid\/\d+/', $url)){
        $url=str_replace(array('/Home/Index','/aid'), '', $url);
    }
...
function upload($file, $path = 'image/editormd', $childPath = true)
{

    $new_file_name = get_rand_str(6);
    $config = array("maxSize" => 2000000,
        "exts" 	  => array("jpg","jpeg","png","gif"),
        "rootPath" =>"./Upload/",
        "savePath" =>$path."/".date('Ymd')."/" ,// 设置附件上传（子）目录
        "saveName" => $new_file_name,
        "autoSub" =>false
    );
    $upload = new Think\Upload($config);
    $old_file_name = $_FILES[$file];
    $info = $upload -> uploadOne($old_file_name);


    if($info){

        $filePath = "/Upload/".$info['savepath'];
        $data = ['status_code' => 200, 'message' => '上传成功', 'data' => ['old_name' => $old_file_name, 'new_name' =>$info['savename'], 'path' => $filePath]];
        unset($new_file_name);
        unset($old_file_name);
        return $data;
    }else{
        //echo $upload->getError();
        $data = ['status_code' => 500, 'message' => $upload -> getError()];
        return $data;
    }
...
function upload2($file, $path = 'upload', $childPath = true)
{
